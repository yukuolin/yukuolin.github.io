/* 台股籌碼分析頁：讀取 data/market.json（由 GitHub Actions 每日更新）並渲染盤後籌碼區塊 */
(function () {
    "use strict";
    ...
    function summaryCards(el, cards) {
    ...
    // 頁首籌碼摘要卡：加權指數＋三大法人買賣超
    function renderTopSummary(m) {
        var cards = [];
        if (m.taiex) {
            cards.push({
                label: "加權指數",
        ...
        var inst = m.institutional || [];
        var total = findInst(inst, "合計");
        var foreign = findInst(inst, "外資及陸資");
        var trust = findInst(inst, "投信");
        ...
        summaryCards(document.getElementById("chipTopSummary"), cards);
    }
    ...
    function renderDayTrading(dayTrading) {
        ...
        summaryCards(document.getElementById("dayTradingSummary"), cards);
    }

    // 選擇權 Put/Call 比 + 大額交易人未沖銷部位集中度
    function renderOptions(pcr, largeTraders) {
    ...
    function init() {
        ...
        fetch("./data/market.json").then(function (r) {
            ...
            renderOptions((m.chips || {}).putCallRatio, (m.chips || {}).largeTraders);
